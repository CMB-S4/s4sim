...
rhits = hp.read_map("../sims/outputs/sat_deep/SAT_f090/mapmaker_invcov.fits")
rhits /= np.amax(rhits)
rhits = rhits**2  # Use the square of the hits as weight

# Set North Galactic Latitudes to zero
nside = hp.get_nside(rhits)
pix = np.arange(rhits.size)
lon, lat = hp.pix2ang(nside, pix, lonlat=True)
rot = hp.Rotator(coord="GC")
glat = rot.rotate_map_pixel(lat)
rhits[glat > 0] = 0
