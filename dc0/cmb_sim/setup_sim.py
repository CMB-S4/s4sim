...
for TELESCOPE, (telescope, nside, freqs) in bands.items():
    for freq in freqs:
        fname_out = os.path.join(outdir, f"cmb.{telescope}.f{freq:03}.h5")
        if os.path.isfile(fname_out):
            print(f"Output file exists: {fname_out}")
            continue
        fname_in = f"/pscratch/sd/z/zonca/cmbs4/202305_dc0/combined_cmb_unlensed_dipole/cmbs4_combined_cmb_unlensed_dipole_uKCMB_{TELESCOPE}_f{freq:03}_nside{nside}.fits"
