# Study the performance-based scaling applied in DC11 simulations
...
indir = "/global/cfs/cdirs/cmbs4/awg/lowellbb/data_xx.yy/11a3sat.00"
...
nyear_sim = 20  # performance-based simulation survey length
...
for config in "sat",:
    for first_file in sorted(glob.glob(f"{indir}/*{config}*noise*_0000.fits")):
        ijob += 1
        if ijob % ntask != rank:
            continue
        print(prefix + f"Processing {first_file}")

        band = os.path.basename(first_file).split("_")[3]
        alt_band = alt_bands[band]
        fname_cov = f"/global/cfs/cdirs/cmbs4/AoA/August2022/scaled_outputs/alternative_3/chsat_baseline_s4/cov_{alt_band[-3:]}.fits"
