...
#schedule1.read("schedule_lat_delensing_sun90max+wide.txt"); name1 = "delensing_sun90max"
schedule1.read("schedule_lat_delensing_sun90max_gapfill.txt"); name1 = "delensing_sun90max_gapfill"
# schedule1.read("../lat_wide/schedule_lat_wide.txt"); name1 = "wide"
#schedule1.read("schedule_lat_delensing_sun90bk_gapfill.txt"); name1 = "delensing_sun90bk_gapfill"
...
#schedule2.read("schedule_lat_delensing_sun90bk+wide.txt"); name2 = "delensing_sun90bk"
# schedule2.read("schedule_lat_delensing_sun90bk_gapfill.txt"); name2 = "delensing_sun90bk_gapfill"
schedule2.read("../lat_wide/schedule_lat_wide.txt"); name2 = "wide"
...
def get_hits(name, schedule):
    if modulate:
        fname_radec = f"hitmap_{name}.modulated.npy"
        fname_azel = f"hitmap_{name}.azel.modulated.npy"
    else:
...
ax.plot(np.cumsum(m1[:, a1]), label=f"{name1} : {m1sum[a1]:.0f}")
ax.plot(np.cumsum(m2[:, a2]), label=f"{name2} : {m2sum[a2]:.0f}")
ax.set_xlabel("DOY")
ax.legend(loc="best")

ax = fig.add_subplot(nrow, ncol, 2)
ax.set_title("Daily hits to best 40%")
...
ax.set_title("Cumulative hits to best 40%")
ax.plot(np.cumsum(np.sum(m1[:, mask1], 1)), label=f"{name1} : {np.sum(m1sum[mask1]):.0f}")
ax.plot(np.cumsum(np.sum(m2[:, mask2], 1)), label=f"{name2} : {np.sum(m2sum[mask2]):.0f}")
ax.set_xlabel("DOY")
ax.legend(loc="best")
...
for name, msum, col in [(name1, m1sum, 3), (name2, m2sum, 3 + ncol)]:
    ...
    hp.mollview(
        ...
        sub=[nrow, ncol, col],
        cmap="magma",
        title=f"{name}, fsky={fsky:.03f}",
        unit="Hits",
    )

smask1 = hp.smoothing(m1sum, fwhm=np.radians(2), lmax=256) 
smask2 = hp.smoothing(m2sum, fwhm=np.radians(2), lmax=256)
