...
schedule1.read("schedule_lat_wide.txt"); name1 = "phase2.wide"

schedule2 = toast.schedule.GroundSchedule()
schedule2.read("../../../phase1/scan_strategy/lat_wide/schedule_lat_wide.txt"); name2 = "phase1.wide"
...
ax.set_title("Daily hits to best 60%")
...
ax.set_title("Cumulative hits to best 60%")
...
hp.mollview(mask1, sub=[nrow, ncol, 4], min=0, max=1, cmap="bwr", cbar=False, title=name1, reuse_axes=True, alpha=0.25*mask1)
hp.mollview(pdust, sub=[nrow, ncol, 4 + ncol], max=1e-4, cmap="magma", coord="gc", title="")
hp.mollview(mask2, sub=[nrow, ncol, 4 + ncol], min=0, max=1, cmap="bwr", cbar=False, title=name2, reuse_axes=True, alpha=0.25*mask2)
