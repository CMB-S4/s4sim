...
# This workflow copied from TOAST on 2024-06-12
#
# The modifications made in this workflow require a working install
# of Simons Observatory software, sotodlib.
...
def simulate_data(args, job, toast_comm, telescope, schedule):
    ...
    if ops.sim_hwpss.enabled:
        # We must map the S4 SAT bands onto SO SAT bands
        # for the HWPSS module to work
        for ob in data.obs:
            dets = ob.select_local_detectors(None)
        ...
        log.info_rank("  Simulated HWP-synchronous signal", comm=world_comm, timer=timer)

        ops.mem_count.prefix = "After simulating HWP-synchronous signal"
        ops.mem_count.apply(data)
...
def reduce_data(job, args, data):
    ...
    # Demodulation involves downsampling and produces a new data object
    ...
    for det_data in det_data_keys:
        # Two reduction steps need to occur before demodulation
    ...
    if ops.demodulate.enabled:
        # Demodulate the data
        ops.demodulate.det_data = ";".join(det_data_keys)
        ...
        if hasattr(ops, "binner"):
            ops.binner.stokes_weights = demod_weights
        if hasattr(ops, "binner_final"):
            ops.binner_final.stokes_weights = demod_weights
        log.info_rank("  Demodulated data in", comm=world_comm, timer=timer)

    for det_data in det_data_keys:
        # Apply the rest of the filter stack
