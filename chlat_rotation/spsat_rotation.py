...
if len(sys.argv) > 1:
    ...
    #freq = 220
band = f"f{freq:03}"
fname = f"focalplane_SAT1_SAT_{band}.h5"
...
if freq < 50:
    wbin = (20.0 * u.arcmin).to_value(u.deg)
elif freq < 200:
    wbin = (10.0 * u.arcmin).to_value(u.deg)
else:
    wbin = (5 * u.arcmin).to_value(u.deg)
nbin = None
target = None
ymax = None
...
for iangle, angle in enumerate(angles):
    ...
    if do_plot:
        ...
        fname_plot = f"histograms_SAT_{band}_{angle:0>5.1f}deg.png"
        fig.tight_layout()
...
fig.tight_layout()
fig.savefig(f"fraction_and_rms_SAT_{band}.png")
